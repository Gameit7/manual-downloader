...
def is_matching_torrent(torrent_title: str, romaji: str, english: str, ep: int, synonyms: list = None, is_special: bool = False) -> bool:
    ...
    def is_title_match(anime_title: str, torrent_title_lower: str) -> bool:
        ...
        def check_match(raw_title_str: str) -> bool:
            ...
            t_variants = [
                torrent_title_lower,
                torrent_title_lower.replace("'", ""),
                torrent_title_lower.replace("-", "").replace("'", ""),
                torrent_title_lower.replace("-", " ").replace("'", "")
            ]
...
def get_search_queries(romaji: str, english: str, ep: int, synonyms: list = None, is_special: bool = False, erai_title: str = None) -> list:
    ...
    if erai_title:
        _add_base(clean_and_strip(erai_title))
...
def download_torrent(torrent_source: str, torrent_title: str) -> tuple:
    ...
    # Fast-Path: Check for direct HTTP webseed (e.g. Doomdos file.fumodub.com or BEP 19 webseeds)
    if raw_payload:
        try:
            meta, _ = _bdecode(raw_payload)
            info = meta.get("info", {})
            target_video_filename = None
            if "files" not in info and "name" in info:
                name = info["name"]
                target_video_filename = name.decode("utf-8", errors="replace") if isinstance(name, bytes) else str(name)

            url_list = meta.get("url-list")
            if url_list:
                if isinstance(url_list, bytes):
                    candidate_urls = [url_list.decode("utf-8", errors="replace")]
                    ...
                    candidate_urls = [str(url_list)]

                for cu in candidate_urls:
                    cu = cu.strip()
                    if cu.startswith(("http://", "https://")):
                        parts = urllib.parse.urlsplit(cu)
        ...
        for chunk_start in range(0, len(eps_to_process), BATCH_CHUNK_SIZE):
            ...
            for ep_num in chunk_eps:
                ...
                try:
                    ...
                    tg_data = await upload_to_telegram(fp, filename=safe_name, caption=tg_caption, duration=duration_found)
                    if tg_data:
                        await execute_sql("UPDATE episodes SET telegram_file_id = ?, telegram_message_id = ? WHERE id = ?", [tg_data.get("file_id"), tg_data.get("message_id"), ep_id])
